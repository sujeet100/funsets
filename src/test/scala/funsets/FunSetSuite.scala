...
class FunSetSuite extends FunSuite {
  ...
  test("forall returns true if predicate matched for all elements of set ") {
    new TestSets {
      val s = union(s3, union(s1, s2))
  ...
  test("exists returns true if predicate matches for any element") {
    new TestSets {
      val s = union(s3, union(s1, s2))
  ...
  test("map transforms every value of set") {
    new TestSets {
      val s = union(s3, union(s1, s2))
      val m = map(s, x => x * x)
